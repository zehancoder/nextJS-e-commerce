"use client";
import { cartProductState, viewProductState } from "@/toolkit/slice";
          ...
          <div className="border border-gray-300 p-2 rounded-lg">
            <div className={"w-full px-4 py-3 rounded-lg bg-white"}>
              <div className=" md:w-[60%] sm2:w-[60%] w-[50%] lg:w-[70%] mx-auto overflow-hidden px-2 md:px-4 py-2 md:py-4 relative">
                <img
                  className="w-full "
                  src={selectedImg || products.images[0]}
                  alt=""
                />
              </div>
              <div className="flex justify-between">
                <div className="w-[70%] md:w-[65%] border-r border-gray-400">
                  <div className="flex items-center flex-wrap gap-2">
                    {products.images.map((product, idx) => (
                      <div
                        key={idx}
                        onClick={() => setSelectedImg(products.images[idx])}
                  ...
                  <div className="text-gray-700 px-2 py-3">
                    ...
                    <div className="flex font-lexend items-center text-[15px] md:text-[16px] font-medium gap-2">
                      <p className="text-[#FF6C00]">${products.price}</p>
                ...
                <div className="px-2 py-1.5 text-gray-700 font-alan font-normal text-[12px] md:text-[13px]">
                  ...
                  <p>
                    Weight:{" "}
                    {String(products.weight).padEnd("0", "2") +
                      (products.weight < 10 ? "g" : "kg")}
                  </p>
                  <p>Warrenty: {products.warrantyInformation}</p>
                  <p>Shipping: {products.shippingInformation}</p>
                </div>
              </div>
            </div>
            <div
              className="w-full mt-1.5"
              onClick={() => dispath(cartProductState(products))}
            >
